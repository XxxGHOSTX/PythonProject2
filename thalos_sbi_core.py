...
class TransformerEncoderLayer:
    ...
    def forward(self, x: np.ndarray, attention_mask: Optional[np.ndarray] = None) -> np.ndarray:
        ...
        attention_output = self._layer_norm(
            attention_output, self.layer_norm1_weight, self.layer_norm1_bias
        )

        # Feed-forward with residual
        ffn_output = self.ffn.forward(attention_output)
        ffn_output = self._residual_add(attention_output, ffn_output)
        ffn_output = self._layer_norm(ffn_output, self.layer_norm2_weight, self.layer_norm2_bias)
...
class TransformerModel:
    ...
    def __init__(self, config: Config):
        ...
        self.embedding = EmbeddingLayer(
            config.VOCAB_SIZE, config.EMBEDDING_DIM, config.MAX_POSITION_EMBEDDINGS
        )
        ...
        for i in range(config.NUM_ENCODER_LAYERS):
            self.encoder_layers.append(
                TransformerEncoderLayer(
                    config.EMBEDDING_DIM, config.NUM_ATTENTION_HEADS, config.FFN_HIDDEN_DIM
                )
            )
    ...
    def generate_logits(self, encoded: np.ndarray) -> np.ndarray:
        ...
        logits = (
            np.dot(last_hidden, self.output_projection["weight"]) + self.output_projection["bias"]
        )
        return logits
